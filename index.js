...
// Cron: inactivar usuarios externos vencidos — todos los días a las 00:05
cron.schedule('5 0 * * *', async () => {
  try {
    const result = await organizacionService.inactivarExpirados();
    if (result.desactivados > 0) {
      console.log(`[NexaCore Cron] Externos vencidos: ${result.desactivados} usuarios desactivados.`);
    }
  } catch (err) {
    console.error('[NexaCore Cron] Error al inactivar externos vencidos:', err.message);
  }
});
